...
public class Tr_PickPlace_good_v1 extends RoboticsAPIApplication {
	...
	public void run() {
		...
		while (true) {
			...
			case state_run:
				...
				for (int j = fwo.GetCounter()-1; j >= 0; j--) { 		// loop for going the opposite direction
					if (exitForLoop) {
						exitForLoop = false;
						break;
					}
					System.out.println("Going to Frame "+ j +".");
					movePtpWithTorqueCondition(fwo.GetFrame(j));
					if (fwo.GetFrame(j).hasAdditionalParameter("close Gripper")) {	// going the opposite direction so opposite command
					...
					if (fwo.GetFrame(j).hasAdditionalParameter("open Gripper")) { // going the opposite direction so opposite command
