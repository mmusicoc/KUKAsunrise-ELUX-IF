...
public class Test_BrakeTest extends RoboticsAPIApplication {
    ...
    public void run() {
        ...
        lbr_iiwa.move(ptpHome());											// Go to initial position
        getLogger().info("Start evaluation of torque statistic.");
        TorqueEvaluator evaluator = new TorqueEvaluator(lbr_iiwa);			// Start monitoring for maximum torques
        evaluator.setTorqueMeasured(false);									// Select static model torque values
        ...
        TorqueStatistic maxTorqueData = evaluator.stopEvaluation();			// Stop monitoring for maximum torques and store results
        ...
        for (int axis : axes) {
            try {
                BrakeTest brakeTest = new BrakeTest(axis, maxTorqueData.getMaxAbsTorqueValues()[axis]);		// Start brake test
                ...
                switch(brakeTestResult.getState().getLogLevel()) {			// Untested
					case Info:
        ...
        if (allAxesOk) getLogger().info("Brake test was successful for all axes.");
        else getLogger().error("Brake test failed for at least one axis.");
    }
}