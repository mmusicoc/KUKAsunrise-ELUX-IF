...
public class PickItExtendedApplication extends RoboticsAPIApplication {
	...
	private boolean doRelease() throws InterruptedException {
		...
		while (flexFellowIOGroup.getIsVacuumLow() &&
			   System.nanoTime() - release_start_time_ns < timeout_ns) {
			Thread.sleep(50);
		}
		return !flexFellowIOGroup.getIsVacuumLow();
	}
	...
	public void initialize() {
		...
		bin_configs[0].pickit_product_id = 12;
		bin_configs[0].pickit_setup_id = 7;
	...
	public void run() {
		try{
			...
			while (pickit.pickit_is_running()) {
				...
				if (pick_successful || pickit.getRemainingObj() == 0) {
				...
				int trycount = 0;
				while (!pickit.isReady() && trycount < 30) {
					Thread.sleep(100);
					trycount++;
				}
				if (trycount == 30) {
